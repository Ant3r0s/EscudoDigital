// --- IMPORTANTE: Importamos 'env' de la librería ---
import { pipeline, env } from 'https://cdn.jsdelivr.net/npm/@xenova/transformers@2.17.1';

// --- EL EXORCISMO: Forzamos a la IA a buscar los modelos en el sitio correcto ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CORRECCIÓN: Referencias a los botones que faltaban ---
    ...
    // --- Base de Conocimiento de Amenazas (sin cambios) ---
    ...
    // --- Inicialización del Kernel de IA (sin cambios en la lógica, pero la configuración de 'env' le afecta) ---
    ...
    // --- Bucle Principal y Renderizado (sin cambios en la lógica) ---
    ...
    // --- CORRECCIÓN: Listeners para los botones que faltaban ---
    ...
    // --- LÓGICA PARA EL FONDO DE MATRIX (sin cambios) ---
    const canvas = document.getElementById('matrix-background');
    /* ... (resto del código del canvas es idéntico) ... */
});



// =======================================================================================
// === CÓDIGO COMPLETO Y DESPLEGADO DE SCRIPT.JS PARA EVITAR ERRORES ===
// =======================================================================================
import { pipeline, env } from 'https://cdn.jsdelivr.net/npm/@xenova/transformers@2.17.1';
env.allowLocalModels = false;
env.remoteHost = 'https://huggingface.co';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let ner = null;
    const threatLabels = ['phishing', 'estafa de soporte técnico', 'oferta engañosa', 'estafa', 'sextorsión', 'fraude de caridad', 'notificación de entrega falsa', 'falsa urgencia', 'spam', 'legítimo'];
    const threatExplanations = {'phishing': 'El texto suplanta la identidad de una entidad de confianza (banco, red social) para robar tus credenciales.','estafa de soporte técnico': 'Finge ser un soporte técnico (Microsoft, Apple) para que les des acceso a tu equipo o les pagues por un falso problema.','oferta engañosa': 'Una propuesta (trabajo, premio, inversión) que parece demasiado buena para ser verdad, probablemente lo sea.','estafa': 'Un término general para cualquier intento de engañarte para obtener dinero o información valiosa.','sextorsión': 'Amenaza con publicar supuesto material íntimo tuyo si no realizas un pago.','fraude de caridad': 'Pide donaciones para una causa falsa o se hace pasar por una ONG conocida.','notificación de entrega falsa': 'Simula ser una empresa de paquetería (Correos, Amazon) diciendo que tienes un paquete retenido por una pequeña tasa.','falsa urgencia': 'Táctica usada en muchas estafas para que actúes con pánico y sin pensar.','spam': 'Correo o mensaje no solicitado, generalmente publicitario y enviado de forma masiva.','legítimo': 'El texto no presenta indicadores claros de amenaza y parece ser genuino.',};
    const threatAdvice = {'phishing': ['NUNCA hagas clic en enlaces de correos o SMS de este tipo.','Verifica la dirección del remitente. A menudo contiene errores sutiles.','Accede siempre a tu cuenta escribiendo la URL oficial en el navegador, no desde el enlace.','Ninguna entidad seria te pedirá tu contraseña o datos completos por correo.'],'estafa de soporte técnico': ['Cuelga el teléfono o cierra el chat. Microsoft/Apple NUNCA te contactarán de esta forma.','No instales ningún software que te pidan (AnyDesk, TeamViewer).','Nunca les des el control de tu ordenador.'],'oferta engañosa': ['Desconfía de ofertas que no has solicitado y que prometen mucho por poco esfuerzo.','Nunca pagues por adelantado para un trabajo o para recibir un premio.','Investiga a la empresa o persona que te contacta. Busca opiniones en Google.'],'sextorsión': ['NO PAGUES. Pagar no garantiza que borren nada y te pedirán más dinero.','Denuncia el caso a la policía y guarda todas las pruebas.','Bloquea al remitente y no respondas a sus mensajes.'],'notificación de entrega falsa': ['No pagues ninguna tasa aduanera desde un enlace de SMS/email.','Copia el número de seguimiento y búscalo en la web OFICIAL de la empresa de paquetería.','Elimina el mensaje. Es un intento de robar los datos de tu tarjeta.'],'estafa': ['Si suena demasiado bueno para ser verdad, casi seguro que no lo es.','Nunca envíes dinero a alguien que no conoces en persona.','No compartas información personal o financiera con desconocidos que te contactan de la nada.'],'falsa urgencia': ['Tómate un respiro. Las estafas buscan que actúes con pánico y sin pensar.','Contacta con la supuesta entidad (tu banco, tu jefe) a través de un canal oficial que tú conozcas, no el que te proporcionan en el mensaje.','Cuestiona por qué necesitan que actúes AHORA MISMO. Las empresas serias dan plazos razonables.'],'fraude de caridad': ['Verifica que la ONG es real buscando su web oficial en Google.','Dona siempre a través de los canales oficiales de la organización, no a través de enlaces en correos.'],'spam': ['No respondas al mensaje, ni siquiera para pedir que te den de baja.','Marca el mensaje como Correo no deseado o Spam en tu gestor de correo.','No hagas clic en ningún enlace ni descargues archivos adjuntos.']};
    const threatKeywords = {'urgente': 15,'inmediato': 15,'inmediata': 15,'ahora mismo': 15,'última oportunidad': 15,'dispone de 24 horas': 15,'12 horas': 15,'tiempo limitado': 10,'cuenta bancaria': 20,'datos bancarios': 20,'transferencia': 15,'iban': 20,'tarjeta de crédito': 15,'pago': 10,'contraseña': 15,'credenciales': 15,'verificar tu cuenta': 20,'actividad sospechosa': 15,'cuenta suspendida': 15,'no puedo atender llamadas': 25,'móvil apagado': 25,'solo por email': 20,'directora financiera': 10,'ha ganado un premio': 20,'lotería': 20,'herencia': 20,'oferta de trabajo': 10,'haga clic': 10,'en el siguiente enlace': 10,'paquete': 15,'entrega': 15,'envío': 15,'tasas de aduana': 25,'no ha podido ser entregado': 20,'soporte técnico': 25,'virus detectado': 30,'troyano': 30,'software espía': 30,'licencia ha sido suspendida': 25,'llame inmediatamente': 20,'microsoft': 10,'windows': 10};
    async function initializeKernel() {status.textContent = '> STATUS: LOADING NEURAL CLASSIFIER...';classifier = await window.pipeline('zero-shot-classification', 'Xenova/bart-large-mnli');status.textContent = '> STATUS: LOADING NER PAYLOAD EXTRACTOR...';ner = await window.pipeline('token-classification', 'Xenova/bert-base-multilingual-cased-ner-hrl');status.textContent = '> STATUS: KERNEL ONLINE. AWAITING INPUT.';analyzeBtn.disabled = false;analyzeBtn.textContent = '[ ANALYZE THREAT ]';}
    analyzeBtn.disabled = true;analyzeBtn.textContent = '[ KERNEL OFFLINE... ]';initializeKernel();
    analyzeBtn.addEventListener('click', async () => {const textToAnalyze = inputText.value;if (textToAnalyze.trim().length < 20) {status.textContent = '> ERROR: INPUT STREAM TOO SHORT.';return;}analyzeBtn.disabled = true;status.textContent = '> EXECUTING THREAT ANALYSIS... PLEASE WAIT.';resultsContainer.classList.remove('hidden');try {const classificationResult = await classifier(textToAnalyze, threatLabels);const nerResult = await ner(textToAnalyze, { group_entities: true });renderVerdict(classificationResult, textToAnalyze);renderNer(textToAnalyze, nerResult);status.textContent = '> ANALYSIS COMPLETE. STANDING BY.';} catch (error) {console.error("Analysis Error:", error);status.textContent = `> KERNEL PANIC: ${error.message}`;} finally {analyzeBtn.disabled = false;}});
    function calculateHeuristicScore(text) {let score = 0;const lowerCaseText = text.toLowerCase();for (const keyword in threatKeywords) {if (lowerCaseText.includes(keyword.toLowerCase())) {score += threatKeywords[keyword];}}return score;}
    function renderVerdict(aiResult, originalText) {const heuristicScore = calculateHeuristicScore(originalText);const maxScore = Math.max(...aiResult.scores);const maxIndex = aiResult.scores.indexOf(maxScore);let bestLabel = aiResult.labels[maxIndex];const bestScore = aiResult.scores[maxIndex];if (heuristicScore > 20 && bestLabel === 'legítimo') {const possibleScams = ['phishing', 'notificación de entrega falsa', 'estafa'];const scamScores = aiResult.labels.map((label, index) => ({label, score: aiResult.scores[index]})).filter(item => possibleScams.includes(item.label));if (scamScores.length > 0) {bestLabel = scamScores.reduce((prev, current) => (prev.score > current.score) ? prev : current).label;}}const scorePercent = (bestScore * 100).toFixed(1);const explanation = threatExplanations[bestLabel];let verdictClass, verdictTitle;if (heuristicScore >= 40 || (bestLabel !== 'legítimo' && bestScore > 0.7)) {verdictClass = 'alert-red';verdictTitle = `[ALERTA MÁXIMA: ${bestLabel.toUpperCase()} (Riesgo Heurístico: ${heuristicScore}pts)]`;} else if (heuristicScore >= 20 || (bestLabel !== 'legítimo')) {verdictClass = 'alert-orange';verdictTitle = `[SOSPECHOSO: ${bestLabel.toUpperCase()} (Confianza: ${scorePercent}% / Riesgo: ${heuristicScore}pts)]`;} else {verdictClass = 'alert-green';verdictTitle = `[LEGÍTIMO (Confianza: ${scorePercent}%)]`;}verdictBox.className = 'verdict-box';verdictBox.classList.add(verdictClass);typewriterEffect(verdictText, `${verdictTitle}\n> ${explanation}`);renderAdvice(bestLabel);saveToHistory(originalText, `${verdictTitle}\n> ${explanation}`, bestLabel);}
    ...
    function renderNer(originalText, entities) {let highlightedText = originalText;entities.sort((a, b) => b.start - a.start);entities.forEach(entity => {if (['URL', 'PER', 'LOC', 'ORG'].includes(entity.entity_group)) {const tag = `<span class="ner-text-highlight" title="Entidad: ${entity.entity_group}">${entity.word}</span>`;highlightedText = highlightedText.substring(0, entity.start) + tag + highlightedText.substring(entity.end);}});nerText.innerHTML = highlightedText.replace(/\n/g, '<br>');}
    function typewriterEffect(element, text) {element.innerHTML = "";let i = 0;const speed = 10;function type() {if (i < text.length) {element.innerHTML += text.charAt(i) === '\n' ? '<br>' : text.charAt(i);i++;setTimeout(type, speed);}}type();}
    function saveToHistory(text, verdict, label) {const history = JSON.parse(localStorage.getItem('escudoDigitalHistory')) || [];const newEntry = {id: Date.now(),date: new Date().toLocaleString('es-ES'),text: text,verdict: verdict,label: label};history.unshift(newEntry);localStorage.setItem('escudoDigitalHistory', JSON.stringify(history));}
    function loadHistory() {renderHistory();}
    function deleteFromHistory(id) {let history = JSON.parse(localStorage.getItem('escudoDigitalHistory')) || [];history = history.filter(entry => entry.id !== id);localStorage.setItem('escudoDigitalHistory', JSON.stringify(history));renderHistory();}
    function renderHistory() {const history = JSON.parse(localStorage.getItem('escudoDigitalHistory')) || [];historyList.innerHTML = '';if (history.length === 0) {historyList.innerHTML = '<p>No hay análisis guardados.</p>';return;}history.forEach(entry => {const item = document.createElement('div');item.className = 'history-item';item.innerHTML = `<div class="history-item-date">${entry.date}</div><p class="history-item-preview">${entry.text}</p><button class="delete-btn" title="Eliminar entrada"><svg viewBox="0 0 448 512" fill="currentColor" width="16" height="16"><path d="M135.2 17.7L128 32H32C14.3 32 0 46.3 0 64S14.3 96 32 96H416c17.7 0 32-14.3 32-32s-14.3-32-32-32H320l-7.2-14.3C307.4 6.8 296.3 0 284.2 0H163.8c-12.1 0-23.2 6.8-28.6 17.7zM416 128H32L53.2 467c1.6 25.3 22.6 45 47.9 45H346.9c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg></button>`;item.addEventListener('click', () => {inputText.value = entry.text;verdictBox.className = 'verdict-box';typewriterEffect(verdictText, entry.verdict);renderAdvice(entry.label);renderNer(entry.text, []);resultsContainer.classList.remove('hidden');historyModal.classList.add('hidden');});const deleteBtn = item.querySelector('.delete-btn');deleteBtn.addEventListener('click', (event) => {event.stopPropagation();deleteFromHistory(entry.id);});historyList.appendChild(item);});}
    ...
    const canvas = document.getElementById('matrix-background');const ctx = canvas.getContext('2d');let w = canvas.width = window.innerWidth;let h = canvas.height = window.innerHeight;let cols = Math.floor(w / 20) + 1;let ypos = Array(cols).fill(0);ctx.fillStyle = '#000';ctx.fillRect(0, 0, w, h);
    function matrix() {ctx.fillStyle = 'rgba(0,0,0,.05)';ctx.fillRect(0, 0, w, h);ctx.fillStyle = 'rgba(240, 240, 240, 0.9)';ctx.font = '15pt ' + getComputedStyle(document.body).fontFamily;ypos.forEach((y, ind) => {const text = String.fromCharCode(Math.random() * 128);const x = ind * 20;ctx.fillText(text, x, y);if (y > 100 + Math.random() * 10000) {ypos[ind] = 0;} else {ypos[ind] = y + 20;}});}
    setInterval(matrix, 50);
    window.addEventListener('resize', () => {w = canvas.width = window.innerWidth;h = canvas.height = window.innerHeight;cols = Math.floor(w / 20) + 1;ypos = Array(cols).fill(0);});
});
